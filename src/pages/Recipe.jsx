...
import { getNotesByRecipeId } from '../utils/localStorageHelpers';
...
const Recipe = () => {
    ...
    useEffect(() => {
        fetchDetails();
        setNotes(getNotesByRecipeId(recipeId));
    }, [recipeId]);
    ...
    const handleAddNote = (noteText) => {
        const updatedNotes = addNoteToRecipe(recipeId, noteText);
        setNotes(updatedNotes);
    }

    const handleUpdateNote = (noteId, newNoteText) => {
        // 调用助手函数更新笔记并获取更新后的列表
        const updatedNotes = updateNoteForRecipe(recipeId, noteId, newNoteText);
        setNotes(updatedNotes); 
    };
    ...
    const handleDeleteNote = (noteId) => {
        ...
        const updatedNotes = deleteNoteForRecipe(recipeId, noteId);
        setNotes(updatedNotes); 
    };
    ...
    return (
        <div className="recipe-container-main">
            ...
            <div className="recipe-notes-section" style={{ marginTop: '40px' }}> 
                <h2>Notes</h2>
                ...
                <div className="notes-list" style={{ marginTop: '20px' }}>
                    {notes.length > 0 ? (
                        ...
                        <p>There are no notes yet. Come and add one!</p>
                    )}
                </div>
            </div> {/* End of recipe-notes-section */}
        </div>
